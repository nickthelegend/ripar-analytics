...
export default function Page() {
  ...
  return (
    <div className="mx-auto max-w-[1080px] px-5 sm:px-8">
      ...
      {/* Ripar's own numbers — zero, and said so */}
      <section className="border-t py-12" style={{ borderColor: "var(--line)" }}>
        <h2 className="text-[15px] font-semibold tracking-tight">Ripar</h2>
        <div
          className="mt-5 rounded-xl border p-6 sm:p-8"
          style={{ borderColor: "var(--line)", background: "var(--panel)" }}
        >
          <p className="max-w-[62ch] text-[16px] leading-relaxed">
            No endpoints are deployed yet, so there is nothing here to report.
          </p>
          <p className="mt-3 max-w-[62ch] text-[14px] leading-relaxed" style={{ color: "var(--ink-2)" }}>
            Ripar is in build. When the first paid endpoint ships, this section fills with
            calls served, USDC settled and the transactions that settled them — each one
            linking to the explorer, exactly like the block rounds above. Until then it
            stays empty, because a dashboard that invents its own numbers is worth less
            than no dashboard.
          </p>
          <dl className="mt-7 flex flex-wrap gap-x-12 gap-y-4">
            {[
              ...
              <div key={k}>
                ...
                <dd className="tnum mt-1 font-mono text-[20px]" style={{ color: "var(--ink-2)" }}>
                  {v}
                </dd>
              </div>
            ))}
      ...
      >
        <span>
          Read live from Algorand MainNet via public AlgoNode endpoints. No account, no key,
